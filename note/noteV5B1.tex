...
\end{notation}
\begin{definition}
Let $\underline{a}\in\C^n$ and $\underline{r}\in(0,\infty)^n$. We define a poly disk around $a$ to be
\begin{equation*}
...
\begin{definition}
A complex line is a function $l:\C\to\C^n$ such that there exists $\underline{a},\underline{w}\in\C^n,w\not=\underline{o}$, such that 
\begin{equation*}
...
We will consider a holomorphic function $f:U\to\C$ where $U$ is a neighborhood of the closure of $D=D_{\underline{r}}(\underline{a})$. Without the loss of generality we assume $\underline{a}=\underline{o}$. Let $\underline{z}\in D$, applying the Cauchy integral to the first variable, we see that
...
Let $f$ be holomorphic on the domain $U$. If $\vert f\vert$ attains a local maximal then $f\equiv c$ to some constant $c\in \C$.
\end{theorem}

\begin{theorem}
Suppose $f$ is holomorphic on a closure $\overline{D}$ of some polydisk $D=D_r(\underline{z}_0)$. For $\underline{z}\in D$, 
\begin{equation*}
\left({\frac 1 {2\pi i}}\right)\int\cdots\int_T {\frac {f(\underline{z})} {(\underline{\zeta}-\underline{z})}}d\underline{\zeta}.
\end{equation*}
...
a_{\underline{\alpha}} = {\frac {D^{\underline{\alpha}f(\underline{z}_0}} {\underline{\alpha}!}}.
\end{equation*}
Series converges at least in the largest polydisk about $D_r(\underline{z}_0)\subset U$ (the boundary is contained in $U$).
\end{theorem}

\begin{theorem}
Holomorphic functions are analytic, in other words, locally developable into a power series.
\end{theorem}

\begin{theorem}[Identity Theorem]
If $f$ is holomorphic on a domain $U$, and if $f$ is identically $0$ in a non-empty open neighborhood $V$ in $U$, then $f$ is identically $0$ on $U$.
\end{theorem}
...
\begin{enumerate}[1).]
\item $\mathcal{O}(\mathcal{G})$ has no zero divisors. (Follows from Identity theorem).
\item We can in
\end{enumerate}
\end{proposition}
...
\begin{theorem}[Hartogs's Kugelsatz]
...
\begin{remark}
Using the theorem above, we conclude that there are no isolated singularities and isolated $0$s. (ie. a holomorphic function on a punctured domain can be extended to that point). The latter follows that if $f(\underline{z}) = 0$, then ${\frac 1 f}(\underline{z})$ has an isolated singularity.
\end{remark}

\begin{proposition}
Let $f\not\equiv0$ be a holomorphic function on a domain $\mathcal{G}$. We have
\begin{equation*}
V(f) = \{z\in\mathcal{G}\:|\: f(z) = 0\}
\end{equation*}
...
Recommended literature : Range Holomorphic functions and integral representation for several complex variables.

\begin{definition}
Let $F:\C^n\supset U\to V\subset \C^m$ be a function given by $F=(f_1,\cdots,f_m)$ is holomorphic if $f_i$ is holomorphic for each $i$. Or equivalently for any $f\in\mathcal{O}(V)$ , the pullback $f\circ F$ is holomorphic on $U$. 
\end{definition}
...
In particular
\begin{equation*}
\det J_F^\C = \vert\det J_F^2\vert ^2\geq 0.
\end{equation*}
\end{proposition}
...
\begin{proof}
  ...
  \begin{align*}
    MA = E, MB = 0.
  \end{align*}
  Thus we concluder $A = M^{-1},B=O$. We get a Cauchy-Riemann equations.
\end{proof}
...
\begin{definition}
  $D$ is called a tangent vector to $\underline{x}_0\in\R^N$, if 
  \begin{enumerate}[1).]
    \item $D:\mathcal{D}(\underline{x}_0)\to\R$ is a linear map,
    \item $D1 = 0$
...
\begin{definition}
  The vector space of tangent vectors at $\underline{x}0$ is called the tangent space at $\underline{x}_0$ and denoted by $T_{\underline{x}_0}$. Its dual space
...
\begin{example}
  ...
  \end{equation*}
  is an element of the contangent space called the total differentiation.
\end{example}
...
\begin{remark}
$1$-forms on $M$ a module over $E^0(M)$, where $E^0(M)$ is a ring of functions.
\end{remark}
...
Lieb volume 3 chapter 3 integration \\
...
\begin{proposition}
  ...
  \begin{enumerate}[1).]
    ...
    \item commutativetiy for $\wedge$ holds,
...
\begin{example}
  \begin{equation*}
    dx_1\wedge dy_1\wedge\cdots\wedge dx_n\wedge dy_n = -({\frac 1 {n}})^n dz_1\wedge d\overline{z}_1\wedge\cdots\wedge dz_n\wedge d\overline{z}_n.
  \end{equation*}
  This is called the volume form $dV$. 
  \begin{equation*}
    \int_MdV = \int d\lambda(z) = \lambda(M),
  \end{equation*}
  where $\lambda$ is the Lebesgue measure.
\end{example}

\begin{example} The Cauchy kernel.
  \begin{equation*}
    C(\underline{\zeta},\underline{z}) = \left({\frac 1 {2\pi i}}\right)^n {\frac {d\underline{\zeta}} {(\underline{\zeta}-\underline{z})}}.
  \end{equation*}
  This is. a form of type $(n,0)$ in $\underline{\zeta}$. 
  \begin{equation*}
    \C^n\times \C^n - \Delta,
  \end{equation*}
...
\begin{proposition}[Transformation Rules]
  ...
  \begin{equation*}
    a\circ F = (\sum_{I,J} a_{I,J} dw^I\wedge d\overline{w}^J)\circ F  = \sum_{I,J} a_{I,J}\circ F (dw^I\circ F)\wedge (d\overline{w}^J\circ)
  \end{equation*}
...
\end{theorem}
\begin{proof}
  By Weierstrass Vorbereitungs satz, we may assume that $g=e\omega$ where $\omega$ is the Weierstrass polynomial of order $k$. Without the loss of genearlity, we may assume $e = 1$. Thus ,we have,
  \begin{equation*}
    g = z_n^k\exp(D),
  \end{equation*}
