...
\begin{definition}
  ...
  \par A distribution is a linear continuous map $f:\mathcal{D}\to\R$ with respect to the topology given above. Thus the space of distributions of $\mathcal{D}$ is precisely the continuous dual $\mathcal{D}'$ of $\mathcal{D}$.
  We further introduce a topology on $\mathcal{D}'$ by $(f_n)_{n\in \N}$ converges to $0$ in $\mathcal{D}'$ if $f_n(t)\to 0$ for each $t\in \mathcal{D}$. 
\end{definition}

\begin{example}
  Let $f$ be a continuous function, then we define,
  \begin{equation*}
    f(t) = \int_{-\infty}^\infty f(x)t(x)dx.
  \end{equation*}
  From the construction, one sees that for $f:\R\to\R$ to induce a distribution it is enough to assume that $f$ is locally integrable.
\end{example}

\begin{definition}
  The Dirac $\delta$-distribution at the origin is a distribution such that 
  \begin{equation*}
    \delta(f) = f(0).
  \end{equation*}
\end{definition}
...
\begin{definition}
  Let us take $E=\mathcal{D}$ the set of test functions (more generally, we can take $\mathcal{D}'$ the space of distributions). We define for $f,q\in \mathcal{D}'_{p,q}$ by 
  \begin{equation*}
...
\begin{definition}
  We will denote $L^2_{p,q,\loc}(\G)$ to be the set of locally square integrable functions/forms on $\G$. We also define,
  \begin{equation*}
...
\begin{proposition}
  $\mathcal{D}_{pq}$ is dense in $L^2_{pq}(\G,\varphi)$.
\end{proposition}
...
\end{proof}
 ...
 \begin{definition}
    We define $\mathcal{N}_T$ to be the null space of $T$ and $\mathcal{R}_T$ to be the range of $T$. 
  \end{definition}
