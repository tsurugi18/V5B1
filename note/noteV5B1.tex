...
\end{tikzcd}
  ...
  \end{proof}
  What we want to do now is to replicate the calculation we have done before to calculate adjoints but with only one weight function $\varphi$. 
  ...
  \begin{equation*}
    ...
    \end{equation*}
    \begin{theorem}
      Let $\G\compembed\C^n$ be a pseudo convex domain. Then there is a constant $c>0$, only depending on the diameter of $\G$, such that for any $f\in L^2_{0,q+1}(\G)$, $\cbd f=0$, there is $u\in L_{0,q}^2(\G)$, with $\cbd u=f$ and $\norm{u}\leq c\norm{f}$.
