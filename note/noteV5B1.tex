...
\begin{definition}
  ...
  This $B$ above is called the Bochner-Martinelli formel which is define on $\C^n\times \C^n$ except the diagonal. 
\end{definition}
...
\begin{remark}
  Unfortunately, the above statement fails for the surjectivity.
...
\end{tikzcd}
  \end{center}
  is exact if for any open set $U$ of $X$, the sequence 
  \begin{center}
    \begin{tikzcd}
0 \arrow[r] & \cA(U) \arrow[r] & \cB(U) \arrow[r] & \cC(U) \arrow[r] & 0
\end{tikzcd}
  \end{center}
  is exact.
\end{definition}
...
\end{tikzcd}
  ...
  \begin{theorem}
    ...
    Let $X$ be a paracompact space and $\cA$ be a presheaf. Suppose $\hat{A}=0$ then we have $H^q(X,\cA)=0$ for all $q$.
  \end{theorem}
...
\begin{remark}
  For $X$ any polydisk, we have $H^1(X,\Ouv)=0$.
\end{remark}
...
\end{tikzcd}
  ...
  \begin{center}
    \begin{tikzcd}
0 \arrow[r] & \Z \arrow[r] & \Ouv \arrow[r, "f\mapsto \exp(2\pi if)"] & \Ouv^* \arrow[r] & 0
\end{tikzcd}
  \end{center}
...
\end{tikzcd}
  ...
  \begin{remark}
  The above argument can be further generalized. if $D$ is a domain then $D\mapsto \tilde{D}\in H^1(X,\Ouv^*) \stackrel{\sim}{\mapsto}c(D)\in H^2(X,\Z)$.
...
\section{Solving Cauchy-Riemann equations}
